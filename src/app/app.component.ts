...
export class AppComponent {
  ...
  constructor(
    ...
    network.onDisconnect().subscribe(() => {
      ...
      this.shared.showAlertWithTitle(
    ...
    network.onConnect().subscribe(() => {
      if (!connectedToInternet) {
        ...
        this.shared.showAlertWithTitle(
          "Network connected Reloading Data" + "...",
          "Connected"
        );
      }
    ...
    showAd.subscriptions.add(
      showAd.event.subscribe((data) => {
        this.showInterstitial();
      })
    );

    let openThankYouPage = this.appEventsService.subscribe("openThankYouPage");
    openThankYouPage.subscriptions.add(
      openThankYouPage.event.subscribe((data) => {
        this.zone.run(async () => {
    ...
    let openShippingAddressPage = this.appEventsService.subscribe(
      "openShippingAddressPage"
    );
    openShippingAddressPage.subscriptions.add(
      openShippingAddressPage.event.subscribe((data) => {
        this.zone.run(async () => {
          if (config.appNavigationTabs)
            this.navCtrl.navigateForward("tabs/cart/shipping-address");
          else this.navCtrl.navigateForward("/shipping-address");
        });
      })
    );

    let openDeepLink = this.appEventsService.subscribe("openDeepLink");
    openDeepLink.subscriptions.add(
      openDeepLink.event.subscribe((data) => {
        this.naviagateDeeplink(data);
      })
    );

    let openCategoryPage = this.appEventsService.subscribe("openCategoryPage");
    openCategoryPage.subscriptions.add(
      openCategoryPage.event.subscribe((data) => {
        this.openCategoryPage();
      })
    );

    let openHomePage = this.appEventsService.subscribe("openHomePage");
    openHomePage.subscriptions.add(
      openHomePage.event.subscribe((data) => {
        this.zone.run(async () => {
          this.openHomePage();
        });
      })
    ...
    let openSubcategoryPage = this.appEventsService.subscribe(
      "openSubcategoryPage"
    );
    openSubcategoryPage.subscriptions.add(
      openSubcategoryPage.event.subscribe((data) => {
        this.openSubcategoryPage(data);
      })
    );

    let updateSideMenu = this.appEventsService.subscribe("updateSideMenu");
    updateSideMenu.subscriptions.add(
      updateSideMenu.event.subscribe((data) => {
        this.getLeftItems();
      })
    );
  }
    ...
    if (this.plt.is("ios")) {
      if (this.config.admobIos == 1)
        this.initializeAdmob(
          this.config.admobBanneridIos,
          this.config.admobIntidIos
        );
  ...
  openHomePage() {
    if (this.config.appNavigationTabs) {
      ...
      if (this.config.homePage == 6)
        this.navCtrl.navigateForward("/tabs/home6");
      if (this.config.homePage == 7)
        this.navCtrl.navigateForward("/tabs/home7");
      if (this.config.homePage == 8)
        this.navCtrl.navigateForward("/tabs/home8");
      if (this.config.homePage == 9)
        this.navCtrl.navigateForward("/tabs/home9");
      if (this.config.homePage == 10)
        this.navCtrl.navigateForward("/tabs/home10");
    } else {
      if (this.config.homePage == 1) {
    ...
    if (this.config.appNavigationTabs) {
      if (this.config.categoryPage == 1) {
        this.navCtrl.navigateForward(
          this.config.currentRoute + "/categories/0/0"
        );
      }
      if (this.config.categoryPage == 2) {
        this.navCtrl.navigateForward(
          this.config.currentRoute + "/categories2/0/0"
        );
      }
      if (this.config.categoryPage == 3) {
        this.navCtrl.navigateForward(
          this.config.currentRoute + "/categories3/0/0"
        );
      }
      if (this.config.categoryPage == 4) {
        this.navCtrl.navigateForward(
          this.config.currentRoute + "/categories4/0/0"
        );
      }
      if (this.config.categoryPage == 5) {
        this.navCtrl.navigateForward(
          this.config.currentRoute + "/categories5/0/0"
        );
      }
      if (this.config.categoryPage == 6) {
        this.navCtrl.navigateForward(
          this.config.currentRoute + "/categories6/0/0"
        );
      }
    } else {
      if (this.config.categoryPage == 1) {
  ...
  openSubcategoryPage(parent) {
    ...
    if (this.config.appNavigationTabs) {
      if (this.config.categoryPage == 1) {
        this.navCtrl.navigateForward(
          this.config.currentRoute + "/categories" + i
        );
      }
      if (this.config.categoryPage == 2) {
        this.navCtrl.navigateForward(
          this.config.currentRoute + "/categories2" + i
        );
      }
      if (this.config.categoryPage == 3) {
        this.navCtrl.navigateForward(
          this.config.currentRoute + "/categories3" + i
        );
      }
      if (this.config.categoryPage == 4) {
        this.navCtrl.navigateForward(
          this.config.currentRoute + "/categories4" + i
        );
      }
      if (this.config.categoryPage == 5) {
        this.navCtrl.navigateForward(
          this.config.currentRoute + "/categories5" + i
        );
      }
      if (this.config.categoryPage == 6) {
        this.navCtrl.navigateForward(
          this.config.currentRoute + "/categories6" + i
        );
      }
    } else {
      if (this.config.categoryPage == 1) {
  ...
  openPage(page) {
    ...
    else if (page == "/products/0/0/featured")
  ...
  naviagateDeeplink(value: any) {
    ...
    if (
      this.deepUrl.indexOf("product=") != -1 &&
      value != "openCategoryInShop"
    ) {
      ...
      if (value == "openCategoryInShop") {
        for (let val of this.shared.allCategories) {
          ...
          if (val.slug == slug) {
            ...
            console.log(val);
            this.navCtrl.navigateForward(
              "tabs/search/products/" + val.id + "/0/newest"
            );
          }
        }
      }
  ...
  getSingleProductDetail(slug) {
    ...
    this.config
      .getWoo(
        "products/" + "?" + this.config.productsArguments + "&slug=" + slug
      )
      .then(
        (data: any) => {
          this.loading.hide();
          ...
          this.navCtrl.navigateForward(
            this.config.currentRoute + "/product-detail/" + p.id
          );
        },
        (err) => {
          this.loading.hide();
  ...
  getLeftItems() {
    ...
    tempArray.forEach((v: any, index) => {
      ...
      if (
        this.config.enableWpPointReward == false &&
        v.value == "rewardPointsPage"
      ) {
        tempArray.splice(index, 1);
      }
